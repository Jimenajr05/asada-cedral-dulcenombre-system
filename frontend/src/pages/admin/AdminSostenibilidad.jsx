...
import {
  getSostenibilidadAdmin,
  addImagenGaleria,
  updateImagenGaleria,
  deleteImagenGaleria,
  updateTotalHidrantes,
...
function AdminSostenibilidad() {
  ...
  const cargarDatos = async () => {
    try {
      ...
      alert(error.message || "Error al cargar sostenibilidad");
    } finally {
      setLoading(false);
    }
  };
  ...
  const limpiarImagenForm = () => {
    ...
    if (fileInputRefs.current[galeriaActiva]) {
      fileInputRefs.current[galeriaActiva].value = "";
    }
  };
  ...
  const handleGuardarImagen = async (e) => {
    ...
    if (!imagenForm.alt) {
      alert("Debes escribir el texto alternativo");
      return;
    }
    ...
    try {
      ...
      if (imagenForm.image) {
        formData.append("image", imagenForm.image);
      }
      ...
      alert(error.message || "Error al guardar imagen");
    }
  };
  ...
  const handleEliminarImagen = async (index) => {
    const confirmar = window.confirm("¿Deseas eliminar esta imagen?");
    if (!confirmar) return;

    try {
      await deleteImagenGaleria(galeriaActiva, index);

      if (editandoImagen === index) {
        limpiarImagenForm();
      }
  ...
  const handleGuardarTotalHidrantes = async () => {
    try {
      ...
      await cargarDatos();
      alert("Total de hidrantes actualizado correctamente");
    } catch (error) {
      alert(error.message || "Error al actualizar total de hidrantes");
    }
  };
    ...
    <div className="space-y-8 bg-slate-100 p-7">
      <div>
        ...
        <p className="mt-2 text-lg text-slate-700">
          Administra las imágenes de las galerías de sostenibilidad.
        </p>
      </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mb-6 flex flex-wrap gap-3">
          <button
            type="button"
            onClick={() => cambiarGaleria("culturaHidrica")}
            className={`rounded-2xl px-4 py-3 font-semibold transition ${
              galeriaActiva === "culturaHidrica"
                ? "bg-blue-600 text-white"
                : "bg-slate-200 text-slate-700 hover:bg-slate-300"
            }`}
          >
            Actividades Cultura Hídrica
          </button>

          <button
            type="button"
            onClick={() => cambiarGaleria("mantenimiento")}
            className={`rounded-2xl px-4 py-3 font-semibold transition ${
              galeriaActiva === "mantenimiento"
                ? "bg-blue-600 text-white"
                : "bg-slate-200 text-slate-700 hover:bg-slate-300"
            }`}
          >
          ...
          <button
            type="button"
            onClick={() => cambiarGaleria("hidrantes")}
            className={`rounded-2xl px-4 py-3 font-semibold transition ${
        ...
        {galeriaActiva === "hidrantes" && (
          <div className="mb-6 rounded-2xl border border-slate-200 bg-slate-50 p-5">
            <h3 className="text-lg font-bold text-slate-900 mb-4">
              Total mostrado en la galería de hidrantes
            </h3>

            <div className="flex flex-col gap-4 md:flex-row">
              <input
                type="text"
                value={totalHidrantes}
                onChange={(e) => setTotalHidrantes(e.target.value)}
              ...
              <button
                type="button"
                onClick={handleGuardarTotalHidrantes}
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            ...
            <div>
              ...
              >
                ...
                <input
                  ref={(el) => (fileInputRefs.current[galeriaActiva] = el)}
                  type="file"
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <div className="max-h-[720px] overflow-y-auto pr-2">
                    ...
                    {galeriaActual.images.map((image, index) => (
                        ...
                        <div className="p-4">
                            <p className="line-clamp-2 min-h-[3rem] text-sm text-slate-700">
                            ...
                            <button
                                type="button"
                                onClick={() => handleEditarImagen(image, index)}
                ...
                <div className="py-10 text-center text-slate-500">
                    No hay imágenes registradas en esta galería.
                </div>
                )}
            </div>
        </div>
