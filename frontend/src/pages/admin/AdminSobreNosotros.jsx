...
function AdminSobreNosotros() {
  ...
  const cargarDatos = async () => {
    try {
      ...
      alert(error.message || "Error al cargar Sobre Nosotros");
    } finally {
      setLoading(false);
    }
  };
  ...
  const guardarPeriodo = async () => {
    try {
      ...
      alert(error.message || "Error al actualizar período");
    }
  };
  ...
  const handleGuardarMiembro = async (e) => {
    ...
    if (!miembroForm.nombre || !miembroForm.cargo) {
      alert("Debes completar nombre y cargo");
      return;
    }
    ...
    try {
      ...
      if (miembroForm.foto) {
        formData.append("foto", miembroForm.foto);
      }
      ...
      alert(error.message || "Error al guardar miembro");
    }
  };
  ...
  const handleEliminarMiembro = async (index) => {
    const confirmar = window.confirm("¿Deseas eliminar este miembro?");
    if (!confirmar) return;
  ...
  const handleGuardarCobertura = async (e) => {
    ...
    if (!coberturaForm.valor || !coberturaForm.descripcion) {
      alert("Debes completar valor y descripción");
      return;
    }
    ...
    try {
      ...
      alert(error.message || "Error al guardar cobertura");
    }
  };
  ...
  const handleEliminarCobertura = async (index) => {
    const confirmar = window.confirm(
      "¿Deseas eliminar este dato de cobertura?"
    );
    if (!confirmar) return;
    ...
    try {
      ...
      if (editandoCobertura === index) {
        limpiarCoberturaForm();
      }

      alert("Cobertura eliminada correctamente");
    } catch (error) {
      alert(error.message || "Error al eliminar cobertura");
    }
  };
    ...
    <div className="space-y-8 bg-slate-100 p-7">
      <div>
        ...
        <p className="mt-2 text-lg text-slate-700">
          Administra la junta directiva y la sección de cobertura y alcance.
        </p>
      </div>

      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <h2 className="mb-4 text-2xl font-bold text-slate-900">
          Período de Junta Directiva
        </h2>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div>
            ...
            >
              ...
              <input
                ...
                onChange={handleMiembroFileChange}
                className="hidden"
              />
            </div>
          </div>
        ...
        <div className="mt-8 grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
          {data.miembros.length > 0 ? (
            data.miembros.map((miembro, index) => (
              <div
                key={index}
              ...
              >
                <div className="flex flex-col items-center text-center">
                  <div className="h-24 w-24 overflow-hidden rounded-full bg-slate-200">
                    {miembro.foto ? (
                      <img
                        src={construirUrlImagen(miembro.foto)}
                        alt={miembro.nombre}
                        className="h-full w-full object-cover"
                      />
                    ) : (
